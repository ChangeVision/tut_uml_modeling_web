...
class Section
  class PDFConverterCustomChapterTitle < (Asciidoctor::Converter.for 'pdf')
    ...
    def ink_heading_custom_chapter(node, title, opts={})
      ...
      # テーマファイルのタイトルページのイメージを参照する（細かい話はbook-chapter-titleを参照のこと）
      ...
      indent(-50, 0) do # outdent_section を使っても開始位置がページマージンまで戻らない
        convert ::Asciidoctor::Block.new(node, :image,
                  content_model: :empty,
                  attributes: {
                    'target' => image_path, 'pdfwidth' => '40mm', 'align' => 'left'
                  },
                  pinned: true)
      end
    ...
    def ink_heading_colophon(_node, title, _opts = {})
      ...
      move_down 280  # 奥付にちょうといい位置まで下げる
